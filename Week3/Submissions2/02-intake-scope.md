...
Third, the 8-week timeline creates acute scope discipline pressure that is political as much as technical. Every scope item that cannot produce a visible result inside 8 weeks is exposed to cancellation. The facility preference enrichment project, the nurse database API validation, and the coordinator HITL interface design each have dependencies that may not resolve in 8 weeks. The Wave 1 deliverable — WS1 NLP extraction in brief completion mode (coordinators actively complete partially-filled briefs from day 1 of week 9) plus shared HITL queue infrastructure — is the scope achievable and demonstrable within the window, with a narrow WS2 pilot at week 6 (1 facility, 1 specialty, 2 coordinators) providing an early time-to-fill demonstration for the board. WS2 full HITL rollout is the 12-week follow-on — but must be framed to Marcus as the primary business-value wave that Wave 1 enables, not a deferral.
...
> **Agent design implication:** Wave 1 scope (WS1 NLP extraction in brief completion mode + HITL queue live for coordinators) must be deployable and producing coordinator-facing output by week 8, with a narrow WS2 pilot (1 facility, 1 specialty, 2 coordinators) running by week 6 against a real shift request. WS2 full HITL rollout enters Wave 2 (~week 12); any plan that requires full WS2 deployment inside 8 weeks is scope-unsafe.
...
|-------|--------|-----|-----|--------------------|
| **Wave 1** | Weeks 1–8 | **Intake & Matching Agent — brief completion mode**: extracts high-confidence fields (datetime, facility name via lookup, urgency signal); coordinator completes remaining fields (specialty, credential level, unit type) in HITL queue in under 90 seconds | Intake & Matching Agent development begins; spec and architecture approved; not deployed | WS1 achieves ≥95% extraction accuracy on validation set |
| **Narrow WS2 pilot** | **Week 6** | **Intake & Matching Agent — WS1-lite**: 3 high-confidence fields extracted; coordinator completes 3 remaining fields; outputs stable brief schema | **Intake & Matching Agent — HITL pilot**: generates real shortlist from WS1-lite brief; coordinator selects from HITL queue; agent executes on approval — 1 facility, 1 specialty, 2 coordinators | Pilot evidence informs full Wave 2 rollout |
| **Wave 2 Phase 1** | ~Week 12 | **Intake & Matching Agent — live**: full pipeline; feeds structured briefs to WS2 query | **Intake & Matching Agent — HITL mode**: generates ranked shortlist with credential citations → coordinator selects → agent submits on coordinator approval | Coordinator agreement rate ≥85% on agent top-ranked candidate + zero HR-1 violations over 4 consecutive weeks |
...
**Brief completion mode (WS1):** Agent extracts high-confidence fields (shift datetime, facility name via facility lookup, urgency signal) and pre-populates a structured brief form in the HITL queue. Coordinator opens the queue item and completes the remaining fields (specialty, credential level, unit type) — total coordinator time under 90 seconds versus the current 3–4 minutes from scratch. Agent does not feed the WS2 matching pipeline autonomously until the Wave 2 gate is met. Active from week 9 (Wave 1 go-live).

**WS1-lite (Week 6 pilot):** A constrained version of brief completion mode scoped to the week-6 WS2 pilot. Extracts the same 3 high-confidence fields and outputs the stable brief schema WS2 always expects — the same schema used in Wave 2. WS2 is built to this schema from day 1; only the producer side (WS1) evolves; WS2 is never rewritten.
...
- **Facility preference enrichment (parallel Wave 1 data workstream):** A structured data collection project running in parallel with Wave 1 agent development — not gated on WS1 or WS2 completion. Build method: coordinator annotation of override history for the most recent 6 months of placements (documenting why coordinator selections diverged from credential-qualified candidates) plus structured intake for new facility relationships. Owner: Kim or equivalent coordinator lead (to be confirmed). Estimated duration: 6–9 months to first usable profile set if started in Wave 1. Estimated cost: 1 part-time coordinator-equivalent, ~$32K for 6 months [A-D6-1, confidence: Low — not stated in scenario; consistent with coordinator compensation ranges]. Without this workstream starting in Wave 1, WS2-JtD-3 (optimal candidate selection) remains Human Only indefinitely and the $200M autonomous fill math does not close on schedule.
...
| Facility preference profiles as an agent capability gate | No structured facility preference data exists [D0C: U-3]; profiles cannot be built within the 8-week window and are not a Wave 1 or Wave 2 agent capability; the enrichment project (see §6a) is a parallel data workstream with its own owner — it is in scope as a data project but the agent cannot use preference profiles as decision inputs until they are built and validated | First usable profile set targeted 6–9 months from Wave 1 start; enables WS2-JtD-3 upgrade from Human Only to Human-led + Agent Support |
