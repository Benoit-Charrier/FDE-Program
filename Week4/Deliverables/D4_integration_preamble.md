...
| S-07 | Legal / compliance | PHI data store; write access is the highest-risk operation; agent writes only to defined fields (state, payment_amount, clinical_classification_id, hitl_disposition, rejection_codes, updated_by) | M | H | Write scope must be field-level in integration contract; no bulk delete or schema modification access |
