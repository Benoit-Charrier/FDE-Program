...
## Execution: run in 4 passes — do not attempt in a single session

A single-session attempt at this prompt times out after 2+ hours without completing. Run as four explicitly sequenced passes, each in a separate session. FDE approves each pass before starting the next.

| Pass | Scope | Output file | Session prompt |
|------|-------|-------------|----------------|
| 1 | **Preamble §1–§7** — shared entity definitions, system inventory, gap analysis, risk register, context engineering | `Deliverables/D4_preamble_capability_spec.md` | Run this prompt with instruction: "preamble only (§1–§7)" |
| 2 | **Spec A §0–§11 + §14** — WS1 agent identity, scope, entities, activity catalog, requirements, decision logic, integrations, state model, error handling, ambiguity register | `Deliverables/D4a_capability_spec.md` | Run this prompt with instruction: "Spec A only, §0–§11 and §14, reference preamble by section" |
| 3 | **Spec B §0–§11 + §14** — WS2, same shape as Pass 2 | `Deliverables/D4b_capability_spec.md` | Run this prompt with instruction: "Spec B only, §0–§11 and §14, reference preamble by section" |
| 4 | **Both specs §12–§13** — failure modes and audit/governance for Spec A and Spec B | Append to D4a and D4b | Run this prompt with instruction: "§12 and §13 only, for both Spec A and Spec B" |

**§12 and §13 are required inputs for D7 (validation plan). Do not skip or permanently defer them.**
