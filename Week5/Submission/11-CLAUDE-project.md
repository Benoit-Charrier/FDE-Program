...
| C1a | Client Proposal | `Deliverables/C1a_client_proposal.md` | ✓ Complete — full engagement proposal to Greenfield Health Systems; covers scope, ROI ($732K/yr recovery, 98.3% cost reduction), and recommended agentic pipeline |
| C1b | WS1 Deep Dive | `Deliverables/C1b_WS1_deep_dive.md` | ✓ Complete — operational description of the WS1 agent for Greenfield stakeholders; plain-language walkthrough of the 10-step pipeline |
| C1c | Prototype Scope Decision | `Deliverables/C1c_prototype_scope.md` | ✓ Complete — scope boundary document; justifies WS1 as prototype target; referenced by test comments in `test_ws1_pipeline.py` |
| C12-build | Build Loop Start | `Deliverables/C12_build_loop_start.md` | ✓ Complete — spec completeness assessment before the build loop; Pass 1 (WS1) gap analysis that drove spec fixes before coding began |
| D2C | Volume × Value Analysis | `Deliverables/D2C_volume_value_analysis.md` | ✓ Complete — ATX Phase 4 candidate prioritisation; volume × value scoring that justified WS1 as the highest-value automation target |
| 06 | Capstone Proposal | `Deliverables/06-capstone-proposal.md` + `06-capstone-proposal.html` | ✓ Complete — Gate5a submission-format capstone proposal (markdown + rendered HTML) |
...
| File | Purpose |
|------|---------|
| `Input/build_guidelines.md` | Build session workflow: Plan → Implement → Test; output conventions for agent build sessions |
