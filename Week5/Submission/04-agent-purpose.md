...
|---------|-----------|-----------|-----------------|---------------|---------------|------------|
| T-01 | Claim format parsing and canonical record extraction | Retrieval | Fully agentic | `NormalizedClaimInput` record (delivered by INT Intake Agent) | Rule-based format parser | Low |
...
| ET-07 | (a) Audit record generation (T-11) fails or produces a record with any required field absent (step output, confidence score, matched reference, timestamp, decision rationale); or (b) FM-A-5 governance hard stop — T-09 pre-condition check finds `state ≠ ADMIN_CLEARED`, indicating state corruption or out-of-sequence execution (`trigger_type: GOVERNANCE_VIOLATION`) | HITL exception processor | (a) Partial audit record, list of missing fields by field name, claim ID, pipeline step at which failure occurred; (b) `trigger_type: GOVERNANCE_VIOLATION`, actual state at detection, expected state (`ADMIN_CLEARED`), claim ID — incoming state preserved unchanged for diagnostic use; `payment_amount` never written | 1 hour |
...
> **Detection:** The INT Intake Agent validates the canonical record against a required-fields schema before delivering it to WS1. Any record with a missing required field triggers a `PARSE_FAILED` rejection with a specific error code returned to the provider. A secondary schema validation step is run at the entry point of T-08 (clinical classification) within WS1, since this step's output is the highest-consequence decision in the pipeline.
