...
| T-03 | Eligibility discrepancy resolution | Reasoning | Agent-led + HITL on condition | Eligibility API response, member record, claim date of service | Haiku 4.5, eligibility API | Medium |
...
| T-05 | Coding plausibility assessment | Reasoning | Agent-led + HITL on condition | Diagnosis codes, procedure codes, provider specialty | Haiku 4.5, coding reference table | Medium |
...
| T-07 | Prior auth partial-match resolution | Reasoning + Decision | Agent-led + HITL on condition | Prior auth record, claimed units, authorised units, `PRIOR_AUTH_UNIT_TOLERANCE_PCT` | Haiku 4.5 + arithmetic | Medium |
...
| T-10 | Contract exception handling | Reasoning | Agent-led + HITL on condition | Contract terms document store, claim data, exception conditions | Haiku 4.5, contract document store | **High** |
...
| ET-05 | Coding plausibility assessment (T-05) produces a Haiku 4.5 confidence score below the coding plausibility threshold, or flags a procedure-diagnosis pairing as implausible against the coding reference | HITL coding specialist | Procedure codes, diagnosis codes, provider specialty, Haiku 4.5 output with reasoning chain, relevant code pairing rule reference | 2 hours |
